...
**NOTE** In this work the goal point is set every 2km in lane 2 the car travels. So when the car goes pass this goal point, the new one is set. It is as if the car is chasing this goal point. 

In order to check the cost of these actions, the trajectories are generated by the Path Planning as previoulsy presented. The lowest cost will be chosen for the action. However, we will need to make sure that the action is safe and possible by the checking collision state and the time gap in front and behind our car. 

					[code: cost_functions.h in get_mincost_action(..) lines 310-345] 



The decision making in driving is influenced by the weight factors (see **constant.h**).
...
The car completes the lap of 4.32 miles in about 6 minutes (5.18 mins for 50 mph).
